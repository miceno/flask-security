...
def reset_password(token):
    ...
    if _security.reset_redirect_url:
        ...
        if invalid or expired:
            ...
            if request.method == 'GET':
                ...
                return redirect(get_url('{}?error={}'.format(_security.reset_redirect_url, error_msg)))
            else:
                form.errors = error_msg
                return _render_json(form)
        if request.method == 'GET':
            return redirect(get_url('{}?token={}'.format(_security.reset_redirect_url, token)))
